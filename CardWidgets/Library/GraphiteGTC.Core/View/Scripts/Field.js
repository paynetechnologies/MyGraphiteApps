...
(function (Field, window, document, Common, Cache, Events, Velocity, undefined) {
    ...
    Field.UpdateValue = function (name, value) {
        ...
        // Field
        var isRadioArray = false;
        var field = Common.Get(name);
        if (Common.IsNotDefined(field)) {
            field = Common.QueryAll('input[name="' + name + '"]');
            if (field.length == 0) {
                return;
            }
            isRadioArray = true;
    ...
    Field.OnChange = function (field, fieldValueUiParameter) {
        ...
        if (Common.IsDefined(onChangeEvent.FormToSerialize)) {
            ...
            if (Common.Closest('.gtc-form', field).id != onChangeEvent.FormToSerialize) {
    ...
    function GetFieldIdentifier (field) {
        ...
        if (field.Type == 'RadioField') {
            if (Common.IsDefined(field.OptionDetail.Options)) {
                var option;
